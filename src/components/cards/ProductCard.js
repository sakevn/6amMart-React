		...
		if (isExisted) {
			if (getCurrentModuleType() === "food") {
				...
				if (isExisted?.quantity + 1 <= item?.stock) {
					if (item?.maximum_cart_quantity) {
						if (
							item?.maximum_cart_quantity <= isExisted?.quantity
						) {
							toast.error(t(out_of_limits));
						} else {
							updateMutate(itemObject, {
			...
			>
				...
				{item?.unit_type ? (
					<Typography
						sx={{
							color: (theme) =>
								theme.palette.customColor.textGray,
						}}
					>
						{item?.unit_type}
					</Typography>
			...
			>
				...
				>
					...
					>
						<Typography
							variant={
								horizontalcard === "true" ? "subtitle2" : "h6"
							}
							marginBottom="4px"
							sx={{
								color: (theme) => theme.palette.text.custom,
					...
					<CustomStackFullWidth mt="100px" spacing={1}>
						...
						>
							<Typography
								...
								variant="body2"
							>
								<CustomSpan>{t("Sold")}</CustomSpan> : {sold}{" "}
								{t("items")}
							</Typography>
							<Typography
								fontSize="11px"
				...
				>
					...
					>
						...
						<CustomStackFullWidth justifyContent="center">
							...
							{cardFor === "flashSaleSlider" &&
								verticalCardFlashSliderUi()}
							{cardFor === "food horizontal card" &&
								foodHorizontalCardUi()}
